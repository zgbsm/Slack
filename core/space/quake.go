...
func QuakeApiSearch(o *structs.QuakeRequestOptions) *structs.QuakeResult {
	...
	_, body, err := clients.NewRequest("POST", quakeServerApi, header, bytes.NewReader(bytesData), 999, false, clients.DefaultClient())
	if err != nil {
...
func QuakeUserSearch(token string) int {
	...
	_, body, err := clients.NewRequest("GET", quakeUserApi, header, nil, 999, true, clients.DefaultClient())
	if err != nil {
		return 0
	}
...
func SearchQuakeTips(query string) *structs.QuakeTipsResult {
	...
	_, b, err := clients.NewRequest("POST", quakeTipsApi, header, strings.NewReader(jsonData), 999, true, clients.DefaultClient())
	if err != nil {
		return &qs
	}
	json.Unmarshal(b, &qs)
...
func getShortcuts(o *structs.QuakeRequestOptions) []string {
	...
	_, body, err := clients.NewRequest("GET", "https://quake.360.net/api/search/shortcuts/quake_service_unique", header, nil, 999, true, clients.DefaultClient())
	if err != nil {
		return shortcuts
	}
